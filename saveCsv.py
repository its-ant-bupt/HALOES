...
def saveCsv(path_t, path_x, path_y, path_v, path_yaw, path_a, path_steer, path_steer_rate, init_x, init_y, sampleT,
            exp_name, path_num):
    with open('./Result/5-state-result-{}.csv'.format(path_num), 'w', encoding='utf-8', newline='') as fp:
        # 写
        writer = csv.writer(fp)
        for i in range(len(path_t)):
            writer.writerow([path_t[i], path_x[i], path_y[i], path_yaw[i]])
    with open('./Result/5-state-result-state-{}.csv'.format(path_num), 'w', encoding='utf-8', newline='') as fp:
        # 写
        writer = csv.writer(fp)
        for i in range(len(path_t)):
            writer.writerow([path_t[i], path_x[i], path_y[i], path_v[i], path_yaw[i], path_steer[i]])
    with open('./Result/5-state-result-control-{}.csv'.format(path_num), 'w', encoding='utf-8', newline='') as fp:
        # 写
        writer = csv.writer(fp)
    ...
    ax.legend()
    plt.savefig("./Result/{}-err-traj-{}.svg".format(exp_name, path_num))
